...
import { 
  ...
  Thermometer, Wand2, RefreshCw, X, FileJson, FileText, FileCode, Volume2, StopCircle, Mic, HardDrive, Settings2, Eye
} from 'lucide-react';
...
export const Generator: React.FC<GeneratorProps> = ({ onSave, historyCount, maxHistory }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Mobile Tab Switcher */}
      <div className="lg:hidden flex p-1 bg-zinc-200 dark:bg-zinc-800 rounded-xl mb-4 shrink-0">
          <button 
          ...
          >
              ...
              {output && <span className="w-2 h-2 rounded-full bg-green-500"></span>}
          </button>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 h-full min-h-0">
        ...
        {/* Configuration Panel */}
        <div className={`w-full lg:w-[420px] flex-shrink-0 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl border border-white/20 dark:border-white/5 rounded-3xl p-6 flex-col gap-6 overflow-y-auto lg:max-h-[calc(100vh-120px)] transition-all duration-300 shadow-2xl shadow-black/5 hover:shadow-black/10 ${mobileTab === 'editor' ? 'flex' : 'hidden lg:flex'}`}>
        ...
        {/* Output Panel */}
        <div className={`flex-1 bg-white/40 dark:bg-black/40 backdrop-blur-2xl border border-white/20 dark:border-white/5 rounded-3xl flex-col h-full lg:h-[calc(100vh-120px)] shadow-2xl shadow-black/5 relative overflow-hidden group transition-all duration-500 ${mobileTab === 'preview' ? 'flex' : 'hidden lg:flex'}`}>
          <div className="p-5 border-b border-zinc-200/50 dark:border-zinc-800/50 flex justify-between items-center bg-white/50 dark:bg-zinc-900/80 backdrop-blur-sm z-10 sticky top-0">
              ...
              <div className="flex items-center gap-2 opacity-100 transition-opacity">
                  {output && (
                      ...
                      {!userRating && (
                          <div className="hidden sm:flex gap-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl px-2 py-1.5 ml-3 shadow-sm">
                              {[1,2,3,4,5].map(star => (
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 font-serif text-lg leading-relaxed text-zinc-800 dark:text-zinc-200 prose prose-zinc dark:prose-invert max-w-none scroll-smooth selection:bg-indigo-100 dark:selection:bg-indigo-900/30 selection:text-indigo-900 dark:selection:text-indigo-100">
            {output ? (
